...
Finally, two testbeam campaigns, held approximately two and a half months apart, were conducted to validate the full system, including modules, planes, PPB and LB boards, the Pre-Shower Interlock and Monitoring (PIM), and the higher-level readout board, the PRB. Even though the complete six-plane configuration was not tested, the campaigns used configurations closely mimicking the final setup. These tests were critical not only to assess the detector's performance, but also to stress-test the entire system and identify subtle issues in advance of installation. 
	...
	Plane assembly proceeds only once the modules have passed the aforementioned qualification and calibration procedures. The various performance metrics are then used to inform a careful placement strategy for the modules within each plane.
		...
		\subsection{Threshold Calibration \textcolor{red}{ 4 pages}}
		The first and fondamental step in calibration of an ASIC, once its front-end electronics WP is defined, is finding its discrimination threshold. The value has to be carefully chosen far enough from the noise baseline so that the ASIC would not be triggered by spurious noise events, but close enough so that the sensitivity is not considerably reduced. 
		    ...
		    The typical result obtained after a coarse and fine threshold scan is given in figure \ref{im:Scurve_ELECT03} for module ELECT03 operated at WP2 and WP3. 
		    
		    \begin{figure}[h]
				\centering 
				\includegraphics[width=1.0\linewidth]{files/Scurve_ELECT03_WP2}
				\includegraphics[width=1.0\linewidth]{files/Scurve_ELECT03_WP3}
		...
		Each S-curve was then fitted with a custom Cumulative Distribution Function (CDF) of a Gaussian distribution, i.e. the function usually used to described the noise, so that: 
		\begin{equation}
			f(x) = B + A \cdot \left[1 - \mathrm{erf}\left(\frac{x - \mu}{\sqrt{2} \sigma}\right)\right]
		\end{equation}
		where the parameter $B$ and $A$ are respectively the baseline of the S-curve and its maximum and $\mu$ and $\sigma$ respectively the mean and standard deviation of the Gaussian distribution of the noise. The distribution of the standard deviation of the noise, $\sigma_V$ can be built by extracting the fit parameter for each S-curve. The results are presented independently for both WP in figure \ref{im:noise_distribution_bothWP}.
		
		 \begin{figure}[h]
				\centering 
				\includegraphics[width=1.0\linewidth]{files/noise_distribution_bothWP}
				\caption{}
				\label{im:noise_distribution_bothWP} 
			\end{figure}
		
		
		At first glance, it is obvious that ASICs can have a wide range of
...
% TODO: Plot of the distribution of the threshold value for all the ASIC, also show at the threshold what is the amount of problematic pixels. done 
% TODO: Distribution of the sigma_V extracted from the S curves. done 
